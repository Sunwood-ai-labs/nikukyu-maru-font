...
from kana_features import halfwidth_ligature_map
...
entries += [{'code':base,'glyph':name,'feature':'ccmp','text':chr(base)+chr(mark)}
            for (base,mark),name in halfwidth_ligature_map(font.getGlyphOrder()).items()]
pages=[]
for start in range(0,len(entries),48):
    ...
    body=''.join(f'<article><label>U+{e["code"]:04X} {e["feature"]}</label><div class="glyph" style="font-feature-settings:\'ss01\' {1 if e["feature"]=="ss01" else 0}">{html.escape(e.get("text",chr(e["code"])))}</div></article>' for e in chunk)
...
assert len(entries)==len(font.getGlyphOrder())-1
assert {e['glyph'] for e in entries}==set(font.getGlyphOrder())-{'.notdef'}
